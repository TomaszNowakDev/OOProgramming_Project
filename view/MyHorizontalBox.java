...
public class MyHorizontalBox extends HBox implements Serializable {
	...
	private VBox addVBox() {
		...
		this.recordVisit.setOnAction(new EventHandler<ActionEvent>() {

			@Override
			public void handle(ActionEvent event) {
				Parent root = new AddVisit(AppController.getInstance().getConsultantList().searchPatient(
						AppController.getInstance().getConsultantList().getAllPacient().get(selectedIndex)));
				Stage stage = new Stage();
				stage.setTitle("Record a Visit");
				stage.setScene(new Scene(root, 400, 450));
				stage.show();
		...
		this.vb.setPadding(new Insets(60, 40, 20, 20));
		this.vb.getChildren().addAll(addPatient, editPatient, removePatient, recordVisit);
